...
/** Fork of ImageJ's standard Brightness/Contrast adjuster that computes
	histograms and automatic ranges from all Z/T planes in the current channel.
...
public class ChannelStackContrastAdjuster extends PlugInDialog implements Runnable,
	ActionListener, AdjustmentListener, ItemListener, ImageListener {

	public static final String LOC_KEY = "channel.b&c.loc";
	...
	Thread thread;
	private static ChannelStackContrastAdjuster instance;
	...
	ImageStatistics channelStats;
	int channelStatsImageID;
	int channelStatsChannel;
	int channelStatsStackSize;
	int channelStatsBitDepth;
	int channelStatsBins;
	double channelStatsHistMin;
	double channelStatsHistMax;
	...
	public ChannelStackContrastAdjuster() {
		super("Channel B&C");
	}
	...
	public void run(String arg) {
		...
		if (instance!=null) {
			if (!instance.getTitle().equals(getTitle())) {
				ChannelStackContrastAdjuster ca = instance;
		...
		instance = this;
		IJ.register(ChannelStackContrastAdjuster.class);
	...
	ImageProcessor setup(ImagePlus imp) {
		...
		channelChanged = channel!=previousChannel;
		if (imp.getID()!=previousImageID || type!=previousType || slice!=previousSlice
		|| channel!=previousChannel || frame!=previousFrame)
	...
	void setupNewImage(ImagePlus imp, ImageProcessor ip)  {
		...
		if (bitDepth==16 || bitDepth==32) {
			Roi roi = imp.getRoi();
			imp.deleteRoi();
			ImageStatistics stats = getCachedChannelStackStatistics(imp);
		...
		if (channelChanged && (bitDepth==16 || bitDepth==32)) {
			min2 = defaultMin;
			max2 = defaultMax;
		}
	...
	void plotHistogram(ImagePlus imp) {
		...
		if (balance && (channels==4 || channels==2 || channels==1) && imp.getType()==ImagePlus.COLOR_RGB) {
			...
			if (range!=0 && imp.getProcessor().getMax()==Math.pow(2,range)-1 && !(imp.getCalibration().isSigned16Bit())) {
				stats = getCachedChannelStackStatistics(imp, 256, 0, Math.pow(2,range));
			} else
				stats = getCachedChannelStackStatistics(imp);
		}
	...
	ImageStatistics getChannelStackStatistics(ImagePlus imp) {
		return new StackStatistics(getCurrentChannelImage(imp));
	}

	ImageStatistics getCachedChannelStackStatistics(ImagePlus imp) {
		return getCachedChannelStackStatistics(imp, 0, 0, 0);
	}

	ImageStatistics getCachedChannelStackStatistics(ImagePlus imp, int bins, double histMin, double histMax) {
		int imageID = imp.getID();
		int channel = imp.getChannel();
		int stackSize = imp.getStackSize();
		int bitDepth = imp.getBitDepth();
		boolean defaultHistogram = bins==0 && histMin==0 && histMax==0;
		if (channelStats!=null && imageID==channelStatsImageID
		&& channel==channelStatsChannel && stackSize==channelStatsStackSize
		&& bitDepth==channelStatsBitDepth && bins==channelStatsBins
		&& histMin==channelStatsHistMin && histMax==channelStatsHistMax)
			return channelStats;
		ImagePlus channelImp = getCurrentChannelImage(imp);
		ImageStatistics stats = defaultHistogram
			? new StackStatistics(channelImp)
			: new StackStatistics(channelImp, bins, histMin, histMax);
		channelStats = stats;
		channelStatsImageID = imageID;
		channelStatsChannel = channel;
		channelStatsStackSize = stackSize;
		channelStatsBitDepth = bitDepth;
		channelStatsBins = bins;
		channelStatsHistMin = histMin;
		channelStatsHistMax = histMax;
		return stats;
	}

	ImagePlus getCurrentChannelImage(ImagePlus imp) {
		int width = imp.getWidth();
		int height = imp.getHeight();
		ImageStack source = imp.getStack();
		ImageStack stack = new ImageStack(width, height, imp.getProcessor().getColorModel());
		int nChannels = Math.max(1, imp.getNChannels());
		int nSlices = Math.max(1, imp.getNSlices());
		int nFrames = Math.max(1, imp.getNFrames());
		int channel = Math.min(Math.max(1, imp.getChannel()), nChannels);
		if (nChannels==1) {
			for (int i=1; i<=source.getSize(); i++)
				stack.addSlice(source.getSliceLabel(i), source.getProcessor(i));
		} else {
			for (int t=1; t<=nFrames; t++) {
				for (int z=1; z<=nSlices; z++) {
					int index = imp.getStackIndex(channel, z, t);
					stack.addSlice(source.getSliceLabel(index), source.getProcessor(index));
				}
			}
		}
		ImagePlus channelImp = new ImagePlus("Current channel", stack);
		channelImp.setCalibration(imp.getCalibration());
		Roi roi = imp.getRoi();
		if (roi!=null)
			channelImp.setRoi(roi);
		return channelImp;
	}
	...
	private void propagate(ImagePlus img) {
		if (img.getBitDepth()==24) {
			GenericDialog gd = new GenericDialog("Channel B&C");
	...
	public void imageClosed(ImagePlus imp) {
		if (imp!=null && imp.getID()==previousImageID)
			previousImageID = 0;
	}

	public void imageUpdated(ImagePlus imp) {
		if (done || imp==null || imp!=WindowManager.getCurrentImage())
			return;
		setup(imp);
		updatePlot();
		updateLabels(imp);
...
} // ChannelStackContrastAdjuster class
